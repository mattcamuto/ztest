  ...
  module Index
    class InvertIndex
      ...
      def add(entity_type, tokenized_document)
        ...
        safe_entity_type = safes_str(entity_type)
      ...
      def lookup(entity_type, key, field)
        idx = @inverted_index[safes_str(entity_type)]
        raise(Ztest::Index::InvertIndex::IndexError.new("InvertIndex: index '#{entity_type}' does not exist!")) unless idx

        idx_key = @inverted_index[safes_str(entity_type)][safes_str(key)]
        raise(Ztest::Index::InvertIndex::IndexError.new("InvertIndex: key '#{key}' for index '#{entity_type}' does not exist!")) unless idx_key

        idx_key[safes_str(field)].to_a
      end

      private

      def populate_invert(doc_id, safe_entity_type, k, v)
        @inverted_index[safe_entity_type][safes_str(k)] ||= {}
        Array(v).each do |val|
          @inverted_index[safe_entity_type][safes_str(k)][safes_str(val)] ||= Set.new
          @inverted_index[safe_entity_type][safes_str(k)][safes_str(val)].add(doc_id)
        end
      end
